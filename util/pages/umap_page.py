...
def umap_page():
    ...
    u_name = pd.read_csv("util/data/f_info2.csv",index_col=0)
    umap = pd.read_csv("util/data/umap.csv", sep=',')
    umap_usp =pd.read_csv("util/data/umap_usp.csv", sep=',')
    ...
    with col1:
        ...
        if len(g) < 5: 
            fig = go.FigureWidget(sc_dt_ct(umap_org,i_select,g,u_name))
